...
let lastTs = 0;
let lastMovesAtTs: number[] = []; // Fix for simultaneous moves
let batteryLevel = 0;
...
function parseCubeData(msg: number[]) {
	...
	if (opcode === 0x2) {
		...
		while (todoMoves.length < 10) {
			...
			// If strictly older, stop looking back
			if (hisTs < lastTs) {
				break;
			}
			// If same timestamp, check if we already processed this specific move
		...
		// Filter todoMoves to ensure we don't re-process the head (msg[34]) if it's a duplicate
		// todoMoves contains [Last, Prev, PrevPrev...] order from the push loop?
		// No, `todoMoves` init with [Latest]. Loop PUSHES older moves.
		// So `todoMoves` is [Latest, Old1, Old2...].

		// We need to filter out any move M where (ts < lastTs) or (ts == lastTs && M in lastMovesAtTs)
		const validMoves: [number, number][] = [];
		for (let i = 0; i < todoMoves.length; i++) {
			const [mv, t] = todoMoves[i];
			if (t > lastTs || (t === lastTs && !lastMovesAtTs.includes(mv))) {
				validMoves.push([mv, t]);
			}
		}

		if (todoMoves.length > 1 && validMoves.length < todoMoves.length) {
			// This is expected during overlap
		} else if (todoMoves.length > 1) {
			giikerutil.log('[qiyicube] miss history moves', JSON.stringify(todoMoves), lastTs);
		}

		// Process valid moves from Oldest to Newest
		// validMoves preserves order [Latest, Old1, Old2...]
		...
		for (let i = validMoves.length - 1; i >= 0; i--) {
			...
			// Update lastTs / lastMovesAtTs incrementally
			if (t > lastTs) {
				lastTs = t;
				lastMovesAtTs = [mv];
			} else if (t === lastTs) {
				if (!lastMovesAtTs.includes(mv)) {
					lastMovesAtTs.push(mv);
				}
			}
		}
		...
		if (newFacelet !== curFacelet) {
			giikerutil.log('[qiyicube] facelet mismatch, sync to', newFacelet);
			...
			curCubie = prevCubie;
			prevCubie = tmp;
			// Reset tracker on forced sync? 
			// No, trust timestamp logic unless huge drift.
		} else {
	...
	// lastTs is updated incrementally inside validMoves loop
}
